...
const GET = async ({ url }) => {
  try {
    ...
    const { data: messages } = await supabase.from("chat_messages").select("id, sender, message, created_at").eq("session_id", sessionId).eq("sender", "admin").gt("created_at", after).order("created_at", { ascending: true });
